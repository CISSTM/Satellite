...
let tempInterval = setInterval(function(){ sendThisData()  }, 1000);
...
function sendThisData (){
	send.toGround("temperature = " + temp());
	send.toGround("pressure = " + pressure());
	send.toGround("altitude = " + altitude(startingPress, pressure(), temp()));
}
